...
class Openset_Voc(torch.utils.data.Dataset):
    ...
    #source domain private class: ‘bus’,‘car’,‘cat’,‘chair’,‘cow’,‘table’,and‘dog’.
    ...
    def __init__(self, data_dir, split, use_difficult=False, transforms=None,openset=None,is_source=True):
        ...
        if openset=="ALL":
            cls = Openset_Voc.CLASSES
        elif openset=="0.75":
            cls = Openset_Voc.CLASSES_075
            print("source domain private class: train and tvmonitor")
            print("source domain class: {}".format(cls))
            print("source domain class number (bg): {}".format(len(cls)))
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        elif openset=="0.5":
            cls = Openset_Voc.CLASSES_05
            print("source domain private class: aeroplane, bicycle, bird, boat, and bottle")
            print("source domain class: {}".format(cls))
            print("source domain class number (bg): {}".format(len(cls)))
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        elif openset=="0.25":
            cls = Openset_Voc.CLASSES_025
            print("source domain private class: bus, car, cat, chair, cow, table, and dog")
            print("source domain class: {}".format(cls))
            print("source domain class number (bg): {}".format(len(cls)))
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
        ...
        print("\n")
        print("##################################################")
        print("Dataset Openset_Voc"+openset,":",len(self.ids))
        #add new code to remove empty bboxe s
        self.ids=self.remove_empty_bbox()
        # get_bbox_len after
        print("Filtration Openset_Voc"+openset,":",len(self.ids))
    ...
    def __getitem__(self, index):
        ...
        domain_labels = torch.ones([1]*len(target), dtype=torch.uint8) if self.is_source else torch.zeros([1]*len(target), dtype=torch.uint8)
    ...
    def map_class_id_to_class_name(self, class_id):
        return Openset_Voc.CLASSES[class_id]
